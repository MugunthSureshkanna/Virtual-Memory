...
void palloc_init (size_t user_page_limit)
{
  ...
  init_pool (&user_pool, free_start + kernel_pages * PGSIZE, user_pages,
             "user pool");
  // Initialize frame table with the given number of user pages & address bias
  frametable_init(user_pages, (uint32_t)(free_start + kernel_pages * PGSIZE));
}
...
void palloc_free_page (void *page) { 
  // If it's a user page, deallocate the frame
  if(is_user_vaddr(page)){
    dealloc_frame_addr(page);
  }
